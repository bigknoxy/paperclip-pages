...
<h4 style="margin: 0 0 0.75rem 0; font-size: 1.25rem;">Master Base64 Encoding</h4>
<p style="margin: 0 0 1rem 0; opacity: 0.95;">Join 500+ developers. Get the <strong>Base64 Quick Reference Cheatsheet</strong> + weekly dev tips.</p>
<form id="emailCtaFormBase64Guide" style="display: flex; gap: 0.5rem; max-width: 400px; margin: 0 auto; flex-wrap: wrap; justify-content: center;">
<input type="email" id="emailCtaBase64Guide" placeholder="your@email.com" required style="flex: 1; min-width: 200px; padding: 0.75rem 1rem; border: none; border-radius: 6px; font-size: 1rem;">
...
<script>
document.getElementById('emailCtaFormBase64Guide').addEventListener('submit', function(e) {
  e.preventDefault();
  const email = document.getElementById('emailCtaBase64Guide').value;
  let emails = JSON.parse(localStorage.getItem('ghp_emails') || '[]');
  emails.push({email: email, source: 'base64-guide-intro', timestamp: new Date().toISOString()});
